...
# Copy your functions here (calculate_statistics and other helper functions)
...
# Streamlit App
def main():
    st.set_page_config(page_title="Trade Book Analyzer", layout="wide")
    st.title('Trade Book Analyzer')
    ...
    with col:
        ...
        if uploadedFile is None:
            st.info("👆 Upload a trade csv file first.")
            st.stop()
        originalData = pd.read_csv(uploadedFile)
        originalData["Date"] = pd.to_datetime(originalData["trade_date"])
        # Calculate profit for each trade and create a new column 'profit'
        originalData['profit'] = originalData.apply(lambda row: row['quantity'] * row['price'] if row['trade_type'] == 'sell' else -row['quantity'] * row['price'], axis=1)
        originalData =originalData.groupby('Date')['profit'].sum().reset_index()
        originalData = originalData.rename(columns={'profit':'PnL'})
        ...
        # Sidebar components for date range selection
        # st.write('Date Range Selection')
        start_date = st.date_input('Start Date', originalData['Date'].min())
        end_date = st.date_input('End Date', originalData['Date'].max())
        # Convert date objects to datetime objects
        start_date = pd.to_datetime(start_date)
        end_date = pd.to_datetime(end_date)

        # Filter the DataFrame based on the selected date range
        tradesData = originalData[(originalData['Date'] >= start_date) & (originalData['Date'] <= end_date)]
        selectedGroupCriteria = 'Date'
        ...
        with st.expander("Check Full PnL"):
